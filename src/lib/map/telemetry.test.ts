...
test("map telemetry accepts bounded operational data without coordinates or messages", () => {
  assert.equal(MapClientEventSchema.safeParse({ provider: "amap", kind: "map_ready", stage: "fit_view", pointCount: 4, omittedCount: 0, durationMs: 120 }).success, true);
  ...
  assert.equal(MapClientEventSchema.safeParse({ provider: "amap", kind: "map_runtime_error", stage: "unknown", pointCount: 4, durationMs: 120 }).success, false);
});
